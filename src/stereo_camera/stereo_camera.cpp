...
void StereoCamera::monoTrack(State& cur_state, const cv::Mat& cur_img0, const cv::Mat& cur_img1, const CameraModel& cam0, const CameraModel& cam1,
  ...
  if (keyframes.empty() || last_frame.feature_points.points.size() < PROP_MIN_POINTS) {
		// if(DEBUG)
		{
			if(last_frame.feature_points.points.size() < PROP_MIN_POINTS) std::cout<<"state propagtion has too less points: "<<last_frame.feature_points.points.size()<<std::endl;
		}
  ...
  KeyFrame& lastKF = keyframes.back();
	...
	if(!propagateState(cur_state, cur_img0, lastKF, last_frame, cam0, cur_feature_points)) return;
	...
	if(DEBUG) std::cout<<"propagate dist = "<<t_lastKF2Cur.norm()<<std::endl;

	// create new KeyFrame after moving enough distance
	if (t_lastKF2Cur.norm()>KF_DIST)
	{
		std::cout<<std::endl<<"propagateState: "<<cur_feature_points.points.size()<<" t_lastKF2Cur norm "<<t_lastKF2Cur.norm()<<std::endl;
		...
		if(TEST_STEREO) {
		  ...
		  last_frame.feature_points = keyframe.feature_points;
			...
			if(reconstructAndOptimize(kf_data, lastKF, cam0, cam1, cur_state.pose, new_feature_points, cur_img0, cur_img1)) {

		    // construct new keyframe
				KeyFrame keyframe;
				keyframe = createKeyFrame(cur_state.pose, cur_img0, cur_img1, new_feature_points, cam0);
...
bool StereoCamera::propagateState(State& cur_state, const cv::Mat& cur_img, KeyFrame& lastKF, Frame& last_frame, const CameraModel& cam, FeaturePoints& cur_feature_points) {
	std::clock_t start = std::clock();
  ...
  for(int i=0; i<status.size(); i++){
		...
		lastKF_features_inlier.push_back(lastKF.feature_points.features[i]);
		lastKF_points_inlier.push_back(lastKF.feature_points.points[i], cv::norm(cur_features[i] - cur_features_refined[i]));
  }
	std::cout<<"propagate refine: "<<last_frame.feature_points.features.size()<<" -> "<<lastKF_points_inlier.size()<<std::endl;

	if(float(lastKF_points_inlier.size()) / last_frame.feature_points.features.size() < PROP_PTS_RATIO){
  ...
  int marker_size = prop_img.rows / 50;
	for(int i=0; i<lastKF_points_inlier.size(); i++) {
		...
		cv::Point3f& p = lastKF_points_inlier.points[i];
		...
		if(reproj_err<PROP_PROJ_DIST) {
			lastKF.feature_points.push_back(lastKF_features_inlier[i], lastKF_points_inlier.points[i], reproj_err);
	...
	std::cout<<"propagateState ratio "<<lastKF.feature_points.points.size()<<" / "<<lastKF_points_inlier.size()<<std::endl;
	cv::imshow("propagate projection", prop_img);

	if(float(lastKF.feature_points.points.size()) / lastKF_points_inlier.size() < PROP_PTS_RATIO){
...
bool StereoCamera::reconstructAndOptimize(KFData& kf_data, const KeyFrame& lastKF,
										  ...
										  const cv::Mat& cur_img0, const cv::Mat& cur_img1)
{
  ...
  //store original scale and normalize translation
	...
	float orig_fts_size = kf_data.cur_features.size();
	...
	std::cout<<"reconstructAndBundleAdjust angle= "<<angle<<" ratio= "<<kf_data.points.points.size() << "/" << orig_fts_size<<std::endl;
	if(angle<0.9 || kf_data.points.points.size() / orig_fts_size < BA_INLIER_THRES) return false;
	...
	if(DEBUG) {
	  ...
	  cv::projectPoints(kf_data.points.points, r_vec, kf_data.t_lastKF2Cur, cam0.K, cv::Mat::zeros(1,4,CV_64F), reproj1_pts);
		...
		for(int i=0; i<kf_data.points.points.size(); i++) {
  ...
  double scale_err = scale_optimizer.optimize(kf_data.lastKF_features, kf_data.points, new_scale, cam1, lastKF.img0, lastKF.img1, SCALE_PYMD, SCALE_MAX_STEP);
	std::cout<<"scale optimize err "<<scale_err<<std::endl;
  ...
  scale = new_scale;
	...
	for(int i=0; i<kf_data.points.points.size(); i++) {
		kf_data.points.points[i] = scale*kf_data.points.points[i];
	}
	...
	for(int i=0; i<kf_data.points.size(); i++) {
		cv::Point3f p = kf_data.points.points[i];
		cv::Mat p_cur = (kf_data.R_lastKF2Cur *(cv::Mat_<double>(3,1)<<p.x, p.y, p.z)) + scale*kf_data.t_lastKF2Cur;

		curKF_fts_pts.push_back(kf_data.cur_features[i],
														cv::Point3f(p_cur.at<double>(0,0), p_cur.at<double>(1,0), p_cur.at<double>(2,0)),
														kf_data.points.uncertainties[i]);
	}
	...
	if(REFINE_PIXEL) {
	  ...
	  cv::projectPoints(kf_data.points.points, r_cv, cam1.stereo.t, cam1.K, cv::Mat::zeros(1,4,CV_64F), lastKF_pts_proj);
		...
		float MAX_DISP = 50.0;
		assert(kf_data.lastKF_features.size() == lastKF_pts_proj.size());
		std::vector<uchar> status;
		std::vector<float> err;
